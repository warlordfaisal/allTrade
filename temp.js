...
async function switchDatabase(companyId) {
  try {
    ...
    console.log(`✅ Found Company:`, company); // Close existing connection if exists

    if (companyDbPool) {
      await companyDbPool.end();
    } // Create new pool for the specific company
...
router.post("/switch-database", async (req, res) => {
  ...
  try {
    ...
    console.log("📤 Response from switchDatabase:", response);
...
router.get("/get-company-info", async (req, res) => {
  try {
    // Ensure that Prisma is properly initialized after switching the database
    ...
    const companyInfo = await prisma.company_info.findMany(); // Fetch all company_info records
